...
struct TraceKitTopLevelTests {
  ...
  @Test("Trace treats spans with missing parents as roots")
  func traceTreatsMissingParentAsRoot() throws {
    let traceId = TraceId()
    let missingParent = SpanId.random()
    let child = makeSpan(
      name: "child-only",
      traceId: traceId,
    ...
    let trace = try Trace(fileName: "123456", spans: [child])

    #expect(trace.rootSpans.count == 1)
    #expect(trace.rootSpans.first?.spanId == child.spanId)
  }
