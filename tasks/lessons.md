...
- Verify the target repository path before planning or mutating files; this workspace uses `/Users/chriskarani/CodingProjects/RYNO/Terra`, not any checkout under `Downloads`.
...
- `SpanHandle` is the canonical public handle for workflow/manual lifecycle control; older `TraceHandle` notes are compatibility history and must not be taught as the current API.
...
- When bridging compatibility APIs onto Terra-owned spans, do not bypass operation-scoped callbacks for behavior that carries privacy or policy decisions; composable operations must preserve capture-policy gating.
